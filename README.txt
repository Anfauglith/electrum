...
Last edit: 24.10.2015
Last success with version: 2.5.1
...
brew update
Install python (and thus pip), pyqt-4 and PySide

brew install python pyqt PySide
...
Install py2app (now required to be able to fix the MachOGraph.py issue)
...
# sudo pip uninstall slowaes ecdsa pbkdf2 requests qrcode protobuf dnspython six setuptools
python setup.py sdist
pip install --pre dist/Electrum-*tar.gz
Packages that are already included in the Electrum module and can be uninstalled:

sudo pip uninstall slowaes ecdsa pbkdf2 requests qrcode protobuf dnspython six setuptools
You may want to add the following modules
...
Build error "ImportError: No module named packages"

FIX: #1007

Build error via MachOGraph.py: "TypeError: dyld_find() got an unexpected keyword argument 'loader'"

FIX:

nano -w /usr/local/lib/python2.7/site-packages/macholib/MachOGraph.py
# And apply fix: changed line 49 in macOGraph from loader=loader.filename
...
if [ -f dist/Electrum.app/Contents/Resources/lib/python2.7/packages/requests/utils.py ]; then
    sed -i.bak "s/get_os_ca_bundle_path()/None/g" dist/Electrum.app/Contents/Resources/lib/python2.7/packages/requests/utils.py &&\
    rm -f dist/Electrum.app/Contents/Resources/lib/python2.7/packages/requests/utils.py.bak &&\
    echo "[OK] Patch applied successfully"
else
    sed -i.bak "s/get_os_ca_bundle_path()/None/g" dist/Electrum.app/Contents/Resources/lib/python2.7/*/util.py &&\
    rm -f dist/Electrum.app/Contents/Resources/lib/python2.7/packages/*/util.py.bak &&\
    echo "[OK] Patch applied successfully"
fi
...
Plot History plugin doesn't work (works in newer versions)
