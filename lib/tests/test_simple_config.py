...
class Test_SimpleConfig(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.fermatum_dir = tempfile.mkdtemp()
        # Do the same for the user dir to avoid overwriting the real configuration
        # for development machines with fermatum installed :)
        self.user_dir = tempfile.mkdtemp()

        self.options = {"fermatum_path": self.fermatum_dir}
    ...
    def tearDown(self):
        ...
        shutil.rmtree(self.fermatum_dir)
    ...
    def test_simple_config_command_line_overrides_everything(self):
        ...
        sources"""
        fake_read_system = lambda : {"fermatum_path": "a"}
        fake_read_user = lambda _: {"fermatum_path": "b"}
        read_user_dir = lambda : self.user_dir
        config = SimpleConfig(options=self.options,
                              read_system_config_function=fake_read_system,
                              read_user_config_function=fake_read_user,
                              read_user_dir_function=read_user_dir)
        self.assertEqual(self.options.get("fermatum_path"),
                         config.get("fermatum_path"))

    def test_simple_config_user_config_overrides_system_config(self):
        """Options passed in user config override system config."""
        fake_read_system = lambda : {"fermatum_path": self.fermatum_dir}
        fake_read_user = lambda _: {"fermatum_path": "b"}
        read_user_dir = lambda : self.user_dir
        config = SimpleConfig(options={},
                              ...
                              read_user_config_function=fake_read_user,
                              read_user_dir_function=read_user_dir)
        self.assertEqual("b", config.get("fermatum_path"))

    def test_simple_config_system_config_ignored_if_portable(self):
        """If fermatum is started with the "portable" flag, system
    ...
    def test_simple_config_user_config_is_used_if_others_arent_specified(self):
        ...
        fake_read_system = lambda : {}
        fake_read_user = lambda _: {"fermatum_path": self.fermatum_dir}
        read_user_dir = lambda : self.user_dir
        config = SimpleConfig(options={},
                              read_system_config_function=fake_read_system,
                              read_user_config_function=fake_read_user,
    ...
    def test_cannot_set_options_passed_by_command_line(self):
        ...
        config = SimpleConfig(options=self.options,
                              ...
                              read_user_config_function=fake_read_user,
                              read_user_dir_function=read_user_dir)
        config.set_key("fermatum_path", "c")
        self.assertEqual(self.options.get("fermatum_path"),
                         config.get("fermatum_path"))
    ...
    def test_can_set_options_from_system_config(self):
        ...
        config = SimpleConfig(options={},
                              ...
                              read_user_dir_function=read_user_dir)
        config.set_key("fermatum_path", "c")
        self.assertEqual("c", config.get("fermatum_path"))
    ...
    def test_can_set_options_set_in_user_config(self):
        ...
        config = SimpleConfig(options={},
                              ...
                              read_user_config_function=fake_read_user,
                              read_user_dir_function=read_user_dir)
        config.set_key("fermatum_path", another_path)
        self.assertEqual(another_path, config.get("fermatum_path"))
    ...
    def test_user_config_is_not_written_with_read_only_config(self):
        ...
        contents = None
        with open(os.path.join(self.fermatum_dir, "config"), "r") as f:
            contents = f.read()
...
"""
    ...
    def setUp(self):
        ...
        self.thefile = tempfile.mkstemp(suffix=".fermatum.test.conf")[1]
    ...
    def test_read_system_config_file_does_not_exist(self):
        somefile = "/foo/I/do/not/exist/fermatum.conf"
