...
from electrum_iop.util import UserCancelled, InvalidPassword
...
MSG_ENTER_ANYTHING = _("Please enter a seed phrase, a master key, a list of "
                       "IoP addresses, or a list of private keys")
...
class InstallWizard(QDialog, MessageBoxMixin, BaseWizard):
    ...
    def __init__(self, config, app, plugins, storage):
        ...
        self.setWindowTitle('Electrum-IOP  -  ' + _('Install Wizard'))
        self.app = app
        self.config = config
        ...
        self.set_icon(':icons/electrum-iop.png')
        self.show()
        self.raise_()
    ...
    def run_and_get_wallet(self):
        ...
        if self.storage.requires_split():
            self.hide()
            msg = _("The wallet '%s' contains multiple accounts, which are no longer supported in Electrum-IOP 2.7.\n\n"
        ...
        if self.storage.requires_upgrade():
            self.hide()
            msg = _("The format of your wallet '%s' must be upgraded for Electrum-IOP. This change will not be backward compatible"%path)
            if not self.question(msg):
                return
    ...
    def init_network(self, network):
        message = _("Electrum-IOP communicates with remote servers to get "
